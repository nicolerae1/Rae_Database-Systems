...
--Lab 6
--I tried my best to undrestand these queries the best I could, however with some of them
--I was completely stumped and didnt even know where to begin. I plan to continously work
--on this particular lab over the break to try and understand if possible.
--I am aware however that all labs were due by Friday at 12noon so here is my sad attempt. 
--Number 1
...
where pid =
	(
	  select avg(priceusd) AS PriceAverage
	  from products
	);
...
--Number 7
--A left outer join between two relations, takes all of the values from the first table as well 
--as all of the common values it shares with the second table. If a value in the second table
--does not exist in the first table, null values will be replaced with those spaces when displayed in the result.

select *
from customers LEFT JOIN orders ON
	customers.cid = orders.cid;

--A right outer join between two relations, takes all of the values from the second table as well as 
--all of the common values it shares with the first table. If a value in the first table does not exist
--in the second table, null values will be replaced with those spaces when displayed.
