...
where cid in (select cid
	  from orders
	  where orders.pid='p01' AND orders.pid='p07');
...
where city in (select city
	  from agents
	  where agents.city='Dallas' OR agents.city='New York');
...
--Number 8
--Check constraints are used to limit the values are can be accepted by one
--or more colummns. They are good when you want to limit the value range
--that can be place in a column. An advantage of this constraint is to check the value
--being entered into a record. If the condition returns a false value
--the record violates the constraint and isn't allowed to be entered into the table.
--A good example of would be a constraint to check for null values. Null values can
--be extremely dangerous if entered into a table, therefore having a check to 
--disregard all null values could be helpful. A bad example may be a constraint to check	   
--for a general key. It is important to have a unique and specific constraint. If it is
--not specific, you will not be able to successfully check for a certain value or key.